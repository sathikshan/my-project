...
class ProductionControlFormNextShiftQueue extends palms.exported.framework.ActionForm {
	constructor() {
		super();
		this.nextShiftQueue = new ProductionControlFormNextShiftQueueEntity();
		this.grid = null;
		this.childGrid = null;
		this.filter = null;
		this.userLoggedInRole =  palms.exported.framework.PalmsUIApplication.getUserRoleCode();
		this.headerRecordList = new ProductionCurrentShiftHeaderRecordWidgetsList(this.nextShiftQueue, this);
		this.editorPanel = new EditorPanelNextShiftQueue(this);
	...
	initialize() {
		...
		if (true) {
            ...
            this.grid.onLinkClick = function(columnID, rowIndex) {
                ...
                if(columnID == 'loadTime') {
                    ...
                    selectedRecord.loadTimeCalculations.forEach(function(id) {
                        var entity = nextShiftQueue.loadTimeCalculations.find(x => x.id === id);
                        if(entity)
                            loadTimeCalculations.push(entity);
                    });
                    selectedRecord.plannedLine.forEach(function(id) {
                        var entity = nextShiftQueue.plannedLine.find(x => x.id === id);
                        if(entity)
			...
			// 	// Validation: Check if drag and drop between different status values
			// 	if (fromStatus !== toStatus) {
			// 		alert("Cannot drag and drop between two different status values.");
			// 		return false; // Prevent drag and drop
			// 	}
			...
			// 	// Define arrays for previousShift and currentShift
			// 	let previousShiftRecords = [];
			// 	let currentShiftRecords = [];
			// 	let currentShiftStarted = false;
			
			// 	// Split the records into previousShiftRecords and currentShiftRecords
			...
			// 		// The current shift starts when we detect a `seq` starting from `1`
			// 		if (!currentShiftStarted && record.get('seq') === 1) {
			// 			currentShiftStarted = true;
			// 		}
			
			// 		// Add records to respective arrays
			// 		if (currentShiftStarted) {
			...
			// 	// If no current shift records found, handle as a fallback
			...
			// 		currentShiftRecords = allRecords; // Handle case with only current shift records
			// 	}
			...
			// 	const currentShiftStartIndex = previousShiftRecords.length; // The starting index of currentShiftRecords in allRecords
			...
			// 	// Perform drag and drop only within currentShiftRecords
			// 	const movedRecord = currentShiftRecords.splice(adjustedFromRow, 1)[0]; // Remove the record from currentShiftRecords
			// 	currentShiftRecords.splice(adjustedToRow, 0, movedRecord); // Insert it into the new position
			
			// 	// Reassign `seq` values based on the new order for currentShiftRecords
			// 	for (let i = 0; i < currentShiftRecords.length; i++) {
			// 		if (currentShiftRecords[i] && typeof currentShiftRecords[i].set === 'function') {
			// 			currentShiftRecords[i].set('seq', i + 1); // Set seq starting from 1
			// 		} else {
			// 			console.error("Invalid record while setting seq:", currentShiftRecords[i]);
			// 		}
			// 	}
			
			// 	// Merge previousShiftRecords and currentShiftRecords back together
			// 	const mergedRecords = [...previousShiftRecords, ...currentShiftRecords];
			
			// 	// Clear the grid and add updated records
			// 	instance.grid.removeAll();
			// 	instance.grid.addRecords(mergedRecords);
			// 	instance.grid.renderGrid();
			
			// 	// Now trigger the API call to update the database with the new seq values for currentShiftRecords
			...
			// 	// Build the request body with shift, line, and currentDate
			// 	const requestBody = {
			// 		date: instance.nextShiftQueue.currentDate,
			// 		shift: instance.nextShiftQueue.shift,
			// 		line: instance.nextShiftQueue.line,
			// 		updatedSeqData: updatedSeqData, // Send the array of actualID and seq values
			// 		user: this.userLoggedInId
			// 	};
			// 	const host = new ProductionHostDetails();
			
			// 	// Call the API to update the PartSeq in the database
			// 	const proxy = new palms.exported.framework.entity.entities.customSearchViews.WebServiceProxyCustomSearchViewEntity();
			// 	//proxy.url = "http://localhost:8081/updatePartSeq"; // API endpoint to update the PartSeq in the database
			// 	proxy.url = host.url + "nextShiftQueueUpdatePartSeq";
			...
			// 	return true; // Allow drag and drop
			// };
		...
		if (true) {
			...
			addButton.onClick = () => {
				try {
					const popupDialog = new AddOtherPartsPopupNextShiftQueue(instance); // Pass the whole instance
					popupDialog.setGlassEnabled(true);
					popupDialog.initialize();
					popupDialog.setAnimationEnabled(true);
					popupDialog.setAutoHideEnabled(true);
					...
					if (instance.nextShiftQueue && instance.nextShiftQueue.otherParts) {
						popupDialog.setRecords(instance.nextShiftQueue.otherParts);
					} else {
						console.warn("instance.nextShiftQueue.otherParts is undefined.",popupDialog);
					}
				} catch (error) {
		...
		if (true) {
			...
			this.childGrid.onRowButtonClick = (columnID, rowIndex) => {
				...
				if (selectedRecord) {
					...
					const currentDate = this.nextShiftQueue.currentDate;
					const shift = this.nextShiftQueue.shift;
					const line = this.nextShiftQueue.line;
					...
					proxy.url = host.url + "nextShiftQueueAddFLVTAndPartsBelowSafetyStock"
...
refreshShiftDetails() {
	...
	loadData(filter) {
		...
		proxy.url = host.url + "nextShiftQueueDetails";
		...
		callback.onSuccess = function(responseData) {
			...
			instance.nextShiftQueue.populateFields(responseObj);
	
			instance.headerRecordList.setRecordData(instance.nextShiftQueue);
			
			instance.grid.removeAll(); //nl
			instance.grid.addRecords(instance.nextShiftQueue.grid);
			instance.grid.renderGrid();
			...
			instance.childGrid.addRecords(instance.nextShiftQueue.flvtParts);
