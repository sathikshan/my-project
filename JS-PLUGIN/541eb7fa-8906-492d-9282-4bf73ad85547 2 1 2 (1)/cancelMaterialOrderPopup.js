...
class cancelMaterialOrderPopup extends palms.exported.framework.StickyNote {
        ...
        if (reason.trim()) {
            ...
            callback.onSuccess = function (responseData) {
                ...
                if (!isValid) {
                    ...
                    instance.callMiscellaneousReceiptCan(response);
                }
            }
    ...
    callMiscellaneousReceiptCan(responseData) {
        ...
        let receiptListCan = new palms.exported.framework.webServiceAccess.EntityList();
        const self = this;

        dataArray.forEach(item => {
            let receiptEntityCan = new palms.client.framework.entity.GenericEntity();
            receiptEntityCan.SetInt("WarehouseID", item.WarehouseID);
            receiptEntityCan.SetInt("PrimaryCompanyID", item.PrimaryCompanyID);
            receiptEntityCan.SetInt("WarehousePrimaryCompanyID", item.WarehousePrimaryCompanyID);
            receiptEntityCan.SetInt("CostBucketID", item.CostBucketID);
            receiptEntityCan.SetInt("ReceivedSKUCostID", item.ReceivedSKUCostID);
            receiptEntityCan.SetInt("TransactionTypeID", item.TransactionTypeID);
            receiptEntityCan.SetInt("ReceivedQuantityInBillingUOM", item.ReceivedQuantityInBillingUOM);
            receiptEntityCan.set("ToLocationID", item.ToLocationID); // null
            receiptEntityCan.SetInt("PackTypeID1", item.PackTypeID1);
            receiptEntityCan.SetInt("BillingUOMID", item.BillingUOMID);
            receiptEntityCan.SetDouble("TotalOfLineAmountsInLocalCurrency", item.TotalOfLineAmountsInLocalCurrency);
            receiptEntityCan.SetBoolean("IsFOC", item.IsFOC);
            receiptEntityCan.SetInt("UnitPriceOfBillingUOMInLocalCurrency", item.UnitPriceOfBillingUOMInLocalCurrency);
            receiptListCan.add(receiptEntityCan);
            console.log("miscellaneousReceipt", receiptListCan);
        });
        ...
        receiptWao.performAction(receiptListCan, "CREATE", miscellaneousReceiptCallback);
    }
    callSKUTransactionsService(responseData) {
        ...
        let skuTransactionWebServiceName = "SKUTransactionsFromHandheldAction.asmx" // "SKUTransactionsFromHandheldAction.asmx";
