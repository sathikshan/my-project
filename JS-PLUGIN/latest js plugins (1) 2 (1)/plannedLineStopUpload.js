(function (global) {
	...
	try {
		...
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload".toLowerCase(), "0");
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload\\PlannedLineStop".toLowerCase(), "0");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\PlannedLineStop".toLowerCase(), "TPS");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\PlannedLineStop".toLowerCase(), "SSPCS");
		...
		var PlannedLineStopLoader = new palms.exported.framework.menuUI.MenuLoader();
		PlannedLineStopLoader.onLoad = function(path) {
			...
			new PlannedLineStopForm('http://localhost:8081/uploadPlanLine/PlannedLineStop').initialize();
		};
		...
		palms.exported.framework.menuUI.MenuPanel.addPluginMenuItem(
			new palms.exported.framework.menuUI.PluginMenuItem(
				"setup-CSVUpload-PlannedLineStop", // Unique identifier for the menu item
				"CSV Upload",                // Category or parent menu
				"Planned Line Stop Upload",          	// Display name of the menu item
				PlannedLineStopLoader        				// Loader to execute when the menu item is selected
			)
		);
	}
	catch(e) {
...
class PlannedLineStopForm extends palms.exported.gwt.user.client.ui.SimplePanel {
