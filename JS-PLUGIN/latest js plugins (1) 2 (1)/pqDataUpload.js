(function (global) {
	...
	try {
		...
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload\\PQDataMaster".toLowerCase(), "0");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\PQDataMaster".toLowerCase(), "TPS");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\PQDataMaster".toLowerCase(), "SSPCS");
		...
		var PQDataMasterLoader = new palms.exported.framework.menuUI.MenuLoader();
		PQDataMasterLoader.onLoad = function(path) {
			...
			new PQDataMasterLoaderUpload('http://localhost:8081/upload/PQDataMaster').initialize();
		};
		palms.exported.framework.menuUI.MenuPanel.addPluginMenuItem(
			new palms.exported.framework.menuUI.PluginMenuItem(
				"setup-CSVUpload-PQDataMaster", // Unique identifier for the menu item
				"CSV Upload",                // Category or parent menu
				"PQ Data Upload",          	// Display name of the menu item
				PQDataMasterLoader        				// Loader to execute when the menu item is selected
			)
		);
	}
	catch(e) {
...
class PQDataMasterLoaderUpload extends palms.exported.gwt.user.client.ui.SimplePanel {
