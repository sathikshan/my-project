(function (global) {
	...
	try {
		...
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload\\CSVUpload".toLowerCase(), "0");
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload\\efficiency".toLowerCase(), "0");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\efficiency".toLowerCase(), "TPS");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\efficiency".toLowerCase(), "SSPCS");
		...
		var LineEfficiencyLoader = new palms.exported.framework.menuUI.MenuLoader();
		LineEfficiencyLoader.onLoad = function(path) {
			...
			new LineEfficiencyUploadForm('http://localhost:8081/upload/lineEfficiency').initialize();
			//http://localhost:8090/FileUpload.asmx/Upload
		};
		...
		palms.exported.framework.menuUI.MenuPanel.addPluginMenuItem(
			new palms.exported.framework.menuUI.PluginMenuItem(
				"setup-CSVUpload-efficiency", // Unique identifier for the menu item
				"CSV Upload",                // Category or parent menu
				"Line Efficiency Upload",          	// Display name of the menu item
				LineEfficiencyLoader        				// Loader to execute when the menu item is selected
			)
		);
	}
	catch(e) {
...
class LineEfficiencyUploadForm extends palms.exported.gwt.user.client.ui.SimplePanel {
