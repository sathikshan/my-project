(function (global) {
	...
	try {
		...
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload\\skuMaster".toLowerCase(), "0");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\skuMaster".toLowerCase(), "TPS");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\skuMaster".toLowerCase(), "SSPCS");
		...
		var skuLoader = new palms.exported.framework.menuUI.MenuLoader();
		skuLoader.onLoad = function(path) {
			...
			new skuUpload('http://localhost:8081/upload/skuMaster').initialize();
		};
		palms.exported.framework.menuUI.MenuPanel.addPluginMenuItem(
			new palms.exported.framework.menuUI.PluginMenuItem(
				"setup-CSVUpload-skuMaster", // Unique identifier for the menu item
				"CSV Upload",                // Category or parent menu
				"SKU Master Upload",          	// Display name of the menu item
				skuLoader        				// Loader to execute when the menu item is selected
			)
		);
	}
	catch(e) {
...
class skuUpload extends palms.exported.gwt.user.client.ui.SimplePanel {
