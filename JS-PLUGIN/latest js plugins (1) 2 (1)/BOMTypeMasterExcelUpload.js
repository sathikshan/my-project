(function (global) {
	...
	try {
		...
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload\\bomTypeMaster".toLowerCase(), "0");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\bomTypeMaster".toLowerCase(), "TPS");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\bomTypeMaster".toLowerCase(), "SSPCS");
		...
		var BomTypeLoader = new palms.exported.framework.menuUI.MenuLoader();
		BomTypeLoader.onLoad = function(path) {
			...
			new BomTypeForm('http://localhost:8081/upload/BOMTypeMaster').initialize();
		};
		palms.exported.framework.menuUI.MenuPanel.addPluginMenuItem(
			new palms.exported.framework.menuUI.PluginMenuItem(
				"setup-CSVUpload-bomTypeMaster", // Unique identifier for the menu item
				"CSV Upload",                // Category or parent menu
				"BOMType Master Upload",          	// Display name of the menu item
				BomTypeLoader        				// Loader to execute when the menu item is selected
			)
		);
	}
	catch(e) {
...
class BomTypeForm extends palms.exported.gwt.user.client.ui.SimplePanel {
