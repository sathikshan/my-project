class MaterialProcurementEditorPanel extends palms.exported.gwt.user.client.ui.VerticalPanel {
	...
	PrepareGrid() {
		...
		const refPONumberCC = new palms.exported.framework.grid.ReferencingColumnConfiguration(
			'refPONo', 'Ref PO Number', false, ReferencingColumnType.ComboBoxString, instance.referenceList, 'Ref PO Number', 'refPONo', true);
	...
	addButtons() {
		...
		if (instance.userLoggedInRole === "TPS_LEADER" && this.status !== "Cancelled" && Object.keys(this.recordData).length > 0) {
			if (cycleEndTimeInSec >= currTime) {
		...
		if (instance.userLoggedInRole === "TPS_LEADER" && this.status !== "Cancelled") {
			if (cycleEndTimeInSec >= currTime) {
