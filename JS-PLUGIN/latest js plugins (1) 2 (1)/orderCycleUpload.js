(function (global) {
	...
	try {
		...
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload".toLowerCase(), "0");
		//palms.exported.framework.PalmsUIApplication.setHashTableValue("setup\\CSVUpload\\orderCycle".toLowerCase(), "0");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\orderCycle".toLowerCase(), "TPS");
		palms.exported.framework.PalmsUIApplication.addRoleScreen("setup\\CSVUpload\\orderCycle".toLowerCase(), "SSPCS");
		...
		loader.onLoad = function(path) {
			...
			new OrderCycle('http://localhost:8081/upload/orderCycle').initialize();
		};
		...
		palms.exported.framework.menuUI.MenuPanel.addPluginMenuItem(
			new palms.exported.framework.menuUI.PluginMenuItem(
				"setup-CSVUpload-orderCycle", // Unique identifier for the menu item
				"CSV Upload",                // Category or parent menu
				"Order Cycle Upload",         	// Display name of the menu item
...
class OrderCycle extends palms.exported.gwt.user.client.ui.SimplePanel {
